/*!
  * Application : bonziworld-anony-edition
...
function changelog() {
  new Dialog({
    title: "Changelogs",
    class: "flex_window",
    html: `<iframe src="./changelogs" style="width: 100%; height: 100%;"></iframe>`,
    x: 300,
    y: 400,
    width: 600,
    height: 400,
...
new Dialog({
    ...
    html: `
        <div class="blessed_body">
            ...
            <ul>
                <li> <b>Skins:</b> 2 custom skins
                <li> <b>Hats:</b> 3 extra hats
                <li> <b>Announcements:</b> You can announce to the room. For example: <code>/announce hi</code>
                <li> <b>Polls:</b> You can make a poll to the room. For example: <code>/poll do you like me</code>
            </ul>
            <h3>Skins</h3>
            <div class="roulette">
            ...
            <div class="roulette">
                ...
                <div class="cardhat nopupil" onclick="cmd('hat nopupil')"></div>
  ...
  while (i < text.length) {
    ...
    const tokens = [
      ...
      { token: "$r$", tag: "gay-rainbow" },
      {
        token: "||",
        tag: "gay-spoiler",
        onclick: 'onclick="revealSpoiler(this)"',
      },
      { token: "``", tag: "code" },
      { token: "%%", tag: "marquee" },
    ];
...
function actuallyCreateAccount() {
  return $("#wiz_name")
    ...
    $("#wiz_name")
      .val()
      .match(/Cosmic/gi)
    ? ($("#page_irregularity2").show(),
      void socket.disconnect(),
      console.error(
        "BonziWORLD will not operate when pretending to become an admin nor will it operate when impersonating an admin.\nPlease read the rules for more information."
      ))
    : $("#wiz_guid")
        .val()
        .match(/Seamus/gi) ||
    ...
    : (socket.emit("createAccount", {
      ...
      $("#page_accountWizard").hide(),
      void alert(
        "Successfully registered! Please reload for the special features to take effect..."
      ),
      console.info(
        "Successfully registered! Please reload for the special features to take effect..."
      ));
...
function playmusic1() {
  (music = new Audio($("#media_link").val())), music.play();
}
function stopmusic() {
  music.pause();
}
...
function openbwcoinmenu() {
  $("#page_bwcoins").show();
  $("#bwcoins_count").text(this.userPublic.coins);
}
function updateAds() {
...
function spawn() {
  ...
  var sockets = [];
  sockets.push(bot);
  bot.emit("login", { name: "AnonyBOT (a!help)" });
  const prefix = "a!";
  const botname = "AnonyBOT (" + prefix + "help)";
  const version = "0.1 Alpha";
  var help =
    "AnonyBOT Commands a!help, a!say {TEXT}, a!joke, a!fact, a!botinfo, a!roominfo Sleep Stream Commands a!help, a!execaudio";
  var botinfo = "AnonyBOT Version " + version + ".";
  ...
  setTimeout(() => {
    bot.emit("command", { list: ["color", "blue"] });
  }, 3200);
  setTimeout(() => {
    sendMsg("AnonyBOT is online. Type " + prefix + "help to see commands.");
    setInterval(() => {
      sendMsg("AnonyBOT is online. Type " + prefix + "help to see commands.");
    }, 60000);
  }, 4300);
  ...
  socket.emit("command", {
    list: [
      "useredit",
      {
        target: targetbonzi,
        name: $("#edit_input").val(),
        color: $("#edit_input_2").val(),
        tag: $("#edit_input_3").val(),
      },
    ],
  }),
  $("#edit_input").val("");
...
function unlock() {
  if (!$("#code_input").val()) {
    $("#page_bonuscodes").hide();
    return;
  }
  socket.emit("command", {
    list: ["guess", $("#code_input").val()],
  });
  $("#code_input").val("");
  $("#page_bonuscodes").hide();
}
...
$(document).keydown(function (key) {
  ...
  if (key.ctrlKey && key.shiftKey && key.which == 85) {
    key.preventDefault();
  }
  if (key.which == 123) {
    key.preventDefault();
...
const saved = JSON.parse(
  localStorage.getItem("saved_options") || JSON.stringify(savedDefault)
);
setInterval(function () {
  if (!autosave) return;
...
function bonziAlert(obj) {
  ...
  let b_alert = document
      .getElementById("b_alert")
...
function bonziBroadcast(obj) {
  ...
  let b_broadcast = document
      .getElementById("b_broadcast")
      .content.children[0].cloneNode(true),
...
function fucked() {
  alert("NOPE");
  lol = new Audio("./sfx/no_fuck_off.mp3");
  lol.play(), socket.disconnect("");
}
function errorFatal() {
...
function setup() {
  $("#chat_send").click(sendInput),
    ...
    socket.on("replaceTVWithURL", function (e) {
      function t(e) {
      ...
      if (
        ...
        23 == e.hourAmount || (22 == e.hourAmount && e.minuteAmount >= 9))
      )
            ...
            onStateChange: function (e) {
              if (0 === e.data) {
                ...
                new YT.Player("bonzi_tv_player", {
                  ...
                  events: {
                    onStateChange: function (e) {
                      if (0 === e.data) socket.emit("updatebonzitv");
                    },
                  },
                });
              }
    ...
    socket.on("room", function (e) {
      if ("news" == e.room) {
        ...
        setTimeout(() => {
          document.getElementById("chat_log").style.backgroundColor = "red";
          $("#title").html(
            spooky[Math.floor(Math.random() * spooky.length)] + " Log"
          );
          if (Math.random() < 0.5)
            document.getElementById("chat_log_list").style.backgroundColor =
              "rgba(255,0,0,0.8)";
        ...
        setTimeout(() => {
          ...
          if (Math.random() < 0.5) return;
          new Audio("https://files.catbox.moe/hafxrd.mp3").play();
      ...
      } else if ("bonzi_weather" == e.room)
        $("#bonzi_tv").html(
          "<div id='bonzi_tv_player' style='position: absolute; overflow: hidden; width: 100%; height: 100%; pointer-events: none;'></div>"
        ),
          theme(
            '#content{background-image:url("/img/desktop/logo.tv.png"), url("/img/desktop/bg.png");} #bonzi_tv_yt{background-image:url("/img/desktop/logo.tv.png"), url("/img/desktop/bg.png"); background-position: top right, center; background-repeat: no-repeat;}'
          ),
          (document.getElementById("bonzi_tv").innerHTML = dontUseMyLocation
            ? '<iframe id="bonzi_tv_yt" style="position: absolute; overflow: hidden; width: 100%; height: 100%; pointer-events: none;" src="//v2.weatherscan.net/?long_island" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
            : '<iframe id="bonzi_tv_yt" style="position: absolute; overflow: hidden; width: 100%; height: 100%; pointer-events: none;" src="//v2.weatherscan.net/" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
      else if (e.room == "niggatown") {
        $("#nigga_cont").show();
      } else if ("bonzi_tv" == e.room) {
        $("#room_info").append(
          "<br><font color='red'><h3>BonziTV is in early development and is also a work in progress project. Expect bugs!<br>Report any bugs to the discord or DM Se- I mean, Siobhan.</h3></font>"
        ),
          $("#bonzi_tv").html(
          ...
          theme(
            '#content {background-image:url("/img/desktop/logo.tv.png"); background-repeat: no-repeat; background-position: top-right} #bonzi_canvas {background-image:url("/img/desktop/logo.tv.png"); background-repeat: no-repeat; background-position: top-right}'
          );
        var youtube = new YT.Player("bonzi_tv_player", {
          height: "100%",
          width: "100%",
      ...
      startup = new Audio("./sfx/ab_logon.wav");
      startup.play();
      $("#room_owner")[e.isOwner ? "show" : "hide"](),
        $("#room_public")[e.isPublic ? "show" : "hide"](),
        $("#room_private")[e.isPublic ? "hide" : "show"](),
        $(".room_id").text(e.room);
    }),
    ...
    socket.on("BonziTVTheme", function () {
      $("#room_info").append(
        "<br><font color='red'><h3>BonziTV is in early development and is also a work in progress project. Expect bugs!<br>Report any bugs to the discord or DM Se- I mean, Siobhan..</h3></font>"
      ),
        theme(
    ...
    socket.on("update", function (a) {
      (window.usersPublic[a.guid] = a.userPublic),
        usersUpdate(),
        BonziHandler.bonzisCheck();
      // Update hats if they exist
      if (bonzis[a.guid] && a.userPublic.hat) {
        console.log("Updating hats for user:", a.guid, a.userPublic.hat);
    ...
    socket.on("talk", function (a) {
      ...
      b.cancel(),
        b.runSingleEvent([
          { type: "text", text: a.text, say: a.say || a.text },
        ]);
    }),
    ...
    socket.on("vidlii", function (a) {
      var b = bonzis[a.guid];
      b.cancel(), b.vidlii(a.vid);
    }),
    socket.on("video_flash", function (a) {
      var b = bonzis[a.guid];
      b.cancel(), b.video_flash(a.vid);
    }),
    ...
    socket.on("calladoggis", function (a) {
      var b = bonzis[a.guid];
      b.cancel(), b.calladoggis(a.target);
    }),
    socket.on("wuser", function (a) {
      var b = bonzis[a.guid];
      b.cancel(), b.wuser(a.target);
    }),
    ...
    socket.on("snap", function (a) {
      var b = bonzis[a.guid];
      b.cancel(), b.runSingleEvent(b.data.event_list_snap);
    }),
    /*
  socket.on("fucknak", function (a) {
    var b = bonzis[a.guid];
    b.cancel(), b.runSingleEvent(b.data.event_list_fucknak);
}), */
    ...
    socket.on("nukealte", function (data) {
      var b = bonzis[data.guid];
      if (b) {
      b.explode();
      }
...
var _createClass = (function () {
  ...
  Bonzi = (function () {
    function a(b, c) {
      ...
      _classCallCheck(this, a),
        ...
        this.colorPrev,
        (this.hat = this.userPublic.hat),
        (this.$hat = null), // Will be initialized after DOM element is created
        ...
        (this.$hat = $(this.selElement + " > .bonzi_hat")),
        this.updateName(),
              ...
              items: {
                ...
                quote: {
                  ...
                  callback: function () {
                    ...
                    $("#chat_message")
                      .val('<div data-style="quote">' + d.last + "</div> ")
                      .focus();
                  },
                },
                  wuser: {
                ...
                insults: {
                  ...
                  items: {
                    ...
                    venadoggis: {
                      ...
                      callback: function () {
                        socket.emit("command", {
                          list: ["calladoggis", d.userPublic.name],
                        });
                      },
                    },
                    owo: {
                ...
                menuitemsmod: {
                  ...
                  items: {
                    ...
                    behhif: {
                      ...
                      name: "Behhify",
                      callback: function () {
                        socket.emit("command", { list: ["behh", d.id] });
                      },
                    },
                    doggis: {
                      disabled: function () {
                      ...
                      name: "Doggify",
                      callback: function () {
                        socket.emit("command", { list: ["doggis", d.id] });
                      },
                    },
                    behh: {
                      disabled: function () {
                    ...
                    behha: {
                      ...
                      name: "Nuke (alt)",
                      callback: function () {
                        socket.emit("command", { list: ["nuke2", d.id] });
                      },
                    },
                    behhah: {
                      disabled: function () {
                      ...
                      name: "Nuke (alt again)",
                      callback: function () {
                        socket.emit("command", { list: ["nuke3", d.id] });
                      },
                    },
                    bless: {
                      disabled: function () {
                    ...
                    kike: {
                      ...
                      name: "Moronify",
                      callback: function () {
                        socket.emit("command", { list: ["niggy", d.id] });
                      },
                    },
                    adminify: {
                      disabled: function () {
                      ...
                      name: "Give Admin to",
                      callback: function () {
                        socket.emit("command", { list: ["achive", d.id] });
                      },
                    },
                    demote: {
                      disabled: function () {
                      ...
                      name: "Demote",
                      callback: function () {
                        socket.emit("command", { list: ["demote", d.id] });
                      },
                    },
                    flashem: {
                      disabled: function () {
                      ...
                      callback: function () {
                        socket.emit("command", { list: ["flashed", d.id] });
                      },
                    },
                    vanish: {
                      disabled: function () {
                      ...
                      name: "Vanish a user",
                      callback: function () {
                        socket.emit("command", { list: ["vanish", d.id] });
                      },
                    },
                  },
                },
      ...
      markdownname = markup(this.userPublic.name);
      (this.needsUpdate = !1),
        (document.getElementById("chat_log_list").innerHTML +=
          '<ul><li class="bonzi-message cl-msg ng-scope bonzi-event" id="cl-msg-' +
          this.id +
          '"><span class="timestamp ng-binding"><small style="font-size:11px;font-weight:normal;">' +
          date +
          '</small></span> <span class="sep tn-sep"> | </span><span class="bonzi-name ng-isolate-scope"><span class="event-source ng-binding ng-scope">' +
          markdownname +
          '</font></span></span><span class="sep bn-sep"> has entered teh BWIBE</span>');
      this.runSingleEvent([{ type: "anim", anim: "surf_intro", ticks: 30 }]);
      setTimeout(function () {
        var jump_off_sfx = new Audio("./sfx/jump_off.mp3");
        jump_off_sfx.play();
      }, 3000);
    }
    return (
      _createClass(a, [
        ...
        {
          key: "clearDialog",
          // Code from bonzi.world.
          value: function (tkm, skipVideo, keepOpen) {
            var self = this;
            function _clearDialog() {
              keepOpen ||
                (self.$dialogCont.html(""),
                self.$dialog.removeClass("video-yt"),
                self.$dialog.removeClass("video-file"),
                self.$dialog.removeClass("image"),
                self.$dialog.removeClass("video"),
                self.$dialog.removeClass("autosize"),
                self.$dialog.removeClass("bubble_autowidth"),
                (self.openDialogId = null));
            }
            if (((keepOpen = keepOpen || !1), $(self.$dialog).is(":hidden")))
              return _clearDialog();
            var ckm = String(self.openDialogId);
            "boolean" == typeof tkm
              ? ((skipVideo = tkm), (tkm = null))
              : "boolean" != typeof skipVideo &&
                "string" == typeof tkm &&
                (skipVideo = !1),
              "boolean" != typeof skipVideo && (skipVideo = !1);
            self = this;
            if (
              "string" != typeof tkm ||
              "string" != typeof self.openDialogId ||
              self.openDialogId === tkm
            ) {
              if (
                self.player &&
                "function" == typeof self.player.getPlayerState
              )
                if (skipVideo) {
        ...
        },
        {
          ...
          value: function (a, b, c) {
            ...
            this.$dialogCont[c ? "html" : "text"](a)
              ...
              (this.goingToSpeak = !0),
              {};
            const markdownname1 = markup(this.userPublic.name);
            var toscroll =
              document.getElementById("chat_log_list").scrollHeight -
                document.getElementById("chat_log_list").scrollTop <
              605;
            document.getElementById("chat_log_list").innerHTML +=
              '<ul><li class="bonzi-message cl-msg ng-scope bonzi-event" id="cl-msg-' +
              this.id +
              '"><span class="timestamp ng-binding"><small style="font-size:11px;font-weight:normal;">' +
              date +
              '</small></span> <span class="sep tn-sep"> | </span><span class="bonzi-name ng-isolate-scope"><span class="event-source ng-binding ng-scope"><font color=\'' +
              this.userPublic.color +
              "'>" +
              markdownname1 +
              '</font></span></span><span class="sep bn-sep">: </span><span class="body ng-binding ng-scope" style="color:#dcdcdc;">' +
              a +
              "</span></li></ul>";
            if (toscroll)
              document.getElementById("chat_log_list").scrollTop =
            ...
            if (this.userPublic.voice == "espeak" || espeaktts) {
            ...
            } else if (this.userPublic.voice == "broken") {
              speak.play(
                "[[ " + b,
            ...
            } else if (this.userPublic.voice == "default") {
              this.userPublic.a = new Audio(
                "https://www.tetyys.com/SAPI4/SAPI4?text=" +
                  ...
                  "&pitch=" +
                  Math.max(Math.min(parseInt(this.userPublic.pitch), 400), 50) +
                  "&speed=" +
                  Math.max(Math.min(parseInt(this.userPublic.speed), 250), 50) +
                  ""
              );
            ...
            } else if (this.userPublic.voice == "kinito") {
              this.userPublic.a = new Audio(
                "https://www.tetyys.com/SAPI4/SAPI4?text=" +
                  encodeURIComponent(a) +
                  "&voice=" +
              ...
              this.userPublic.a = new Audio(
                "https://www.tetyys.com/SAPI4/SAPI4?text=" +
                  encodeURIComponent(a) +
                  "&voice=Sam&pitch=" +
                  Math.max(Math.min(parseInt(this.userPublic.pitch), 200), 60) +
                  "&speed=" +
                  Math.max(Math.min(parseInt(this.userPublic.speed), 250), 50) +
                  ""
              );
              ...
              this.userPublic.a = new Audio(
                "https://www.tetyys.com/SAPI4/SAPI4?text=" +
                  encodeURIComponent(a) +
                  "&voice=Mike&pitch=" +
                  Math.max(Math.min(parseInt(this.userPublic.pitch), 226), 60) +
                  "&speed=" +
                  Math.max(Math.min(parseInt(this.userPublic.speed), 250), 50) +
                  ""
              );
              ...
              this.userPublic.a = new Audio(
                "https://www.tetyys.com/SAPI4/SAPI4?text=" +
                  encodeURIComponent(a) +
                  "&voice=Mary&pitch=" +
                  Math.max(Math.min(parseInt(this.userPublic.pitch), 336), 60) +
                  "&speed=" +
                  Math.max(Math.min(parseInt(this.userPublic.speed), 250), 50) +
                  ""
              );
              ...
              this.userPublic.a = new Audio(
                "./voiceforge?text=" +
                  ...
                  "&voice=" +
                  encodeURIComponent(
                    replaceAll(this.userPublic.voice, "voiceforge:", "")
                  )
              );
              this.userPublic.a.play();
              this.userPublic.a.onended = function () {
        ...
        },
        {
            ...
            value: function () {
                ...
                var boomboom = new Audio("./sfx/explodie.mp3");
                boomboom.play();
                var boomboom1 = new Audio("./sfx/explodeyap.mp3");
                boomboom1.play();
          ...
          value: function (a) {
            ...
            this.runSingleEvent([
              ...
              (document.getElementById("chat_log_list").innerHTML +=
                ...
                '</font></span></span><span class="sep bn-sep"> has left teh BWIBE</span>');
            setTimeout(a, 2e3);
          },
        },
        {
            ...
            if (!this.mute) {
              ...
              this.$dialogCont.html(
                "\n\t\t\t\t\t<iframe type='text/html' width='480' height='270' scrolling='no' frameborder='no' allow='autoplay' \n\t\t\t\t\tsrc='https://www.youtube.com/embed/" +
                  vid
                    .replace(/playlist/gi, "videoseries")
                    .replace(/(^\w+:|^)\/.*\.youtube\.com\//, "") +
                  "" +
                  checkurl() +
                  "autoplay=1&modestbranding=1&playsinline=0&showinfo=0&enablejsapi=1&origin=" +
                  window.location.origin +
                  "&widgetid=1&color=purple&theme=dark' \n\t\t\t\t\tstyle='width:480px;height:270px; border-radius: 7px;'\n\t\t\t\t\tframeborder='0'\n\t\t\t\tallow='autoplay; encrypted-media'\n\t\t\t\t\tallowfullscreen='allowfullscreen'\n\t\t\t\t\tmozallowfullscreen='mozallowfullscreen'\n\t\t\t\t\tmsallowfullscreen='msallowfullscreen'\n\t\t\t\t\toallowfullscreen='oallowfullscreen'\n\t\t\t\t\twebkitallowfullscreen='webkitallowfullscreen'\n\t\t\t\t\t></iframe>\n\t\t\t\t"
              ),
                this.$dialog.show();
            }
          },
        },
        {
          ...
          value: function (vid) {
            if (!this.mute) {
              this.$dialog.addClass("bubble_autowidth");
              this.$dialogCont.html(
                "\n\t\t\t\t\t<iframe id='embedplayer' src='https://www.vidlii.com/embed?v=" +
                  vid +
                  "'' width='480' height='270' allowfullscreen scrolling='off' frameborder='0'></iframe>"
              ),
                this.$dialog.show();
            }
          },
        ...
        },
        {
          ...
          value: function (a) {
            this.runSingleEvent([
              { type: "text", text: a + " WANNA HEAR SOMETHING?" },
              {
                type: "text",
                text: "You're a fucking doggis! Ven, Ven a doooogggiiiiiiiiiss!",
                say: "your a fucking doggis! ven, ven a doooooooooooggiiiiiiiiiiiiiiiiiis!",
              },
            ]);
          },
        },
        {
          ...
          value: function (a) {
            this.runSingleEvent([
              ...
              {
                type: "text",
                text: "Welcome to BWIBE!",
                say: "Welcome to BonziWORLD IdealBEHH Edition!",
              },
            ]);
          },
        },
        ...
        */
        ...
        },
        {
          ...
          value: function (hats) {
            ...
            if (typeof hats === "string") {
              this.userPublic.hat = [hats];
            } else if (Array.isArray(hats)) {
              this.userPublic.hat = hats;
            } else {
              this.userPublic.hat = [];
            }
            ...
            this.userPublic.hat.forEach(function (hat, index) {
              ...
              var $hatImg = $("<img>")
                ...
                .on("load", function () {
                  ...
                  console.log("Hat loaded successfully:", hatName);
                })
                .on("error", function () {
            ...
            // If no hats loaded after a short time, hide the container
            setTimeout(() => {
              if (self.$hat.children().length === 0) {
                self.$hat.hide();
              }
            }, 1000);
          },
  ...
  BonziData = {
    ...
    event_list_joke_open: [
      ...
      [{ type: "text", text: "Ok morons it's time for a joke." }],
    ...
    event_list_joke_mid: [
      ...
      [
        {
          type: "text",
          text: "I was fucking a- nope, not gonna say that.",
        },
      ],
      [
        {
          type: "text",
          text: "BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH BEHH ",
        },
      ],
      [
    ...
    event_list_fact_open: [
      [
        {
          type: "html",
          text: "HEY YOU IDIOTS IT'S TIME FOR A <b>FUN</b> FACT",
          say: "hey you idiots its time for a fun fact",
        },
      ],
    ],
    event_list_fact_mid: [
      ...
      [
        ...
        {
          type: "text",
          text: "Dianxar fingers little boys.",
        },
        { type: "anim", anim: "earth_back", ticks: 15 },
        { type: "anim", anim: "grin_fwd", ticks: 15 },
      ],
      [
        ...
        {
          type: "html",
          text: "<img src='./img/icons/other/doggis.png'></img>",
          say: "GRAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA! [[ ",
        },
        {
          type: "text",
          text: "What the fuck! Stop screaming you little shit!",
        },
        {
          type: "html",
          text: "<img src='./img/icons/other/doggis.png'></img>",
          say: "AAALAKAKASALAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA! [[ ",
        },
        {
          type: "text",
          text: "You know what? Screw you guys! I'm gonna play some gacha life!",
        },
      ],
    ],
    event_list_fact_end: [
...
(BonziData.event_list_joke = [
  ...
  (BonziData.event_list_snap = [
    { type: "anim", anim: "shrug_fwd", ticks: 30 },
    {
      type: "text",
    ...
    {
      type: "text",
      text: "I am trained in gorilla warfare and I'm the top sniper in the entire US armed forces.",
    },
    {
      type: "text",
      text: "You are nothing to me but just another target.",
    },
    {
      type: "text",
    ...
    { type: "idle" },
    { type: "anim", anim: "praise_fwd", ticks: 30 },
    {
      type: "text",
    ...
    {
      type: "text",
      text: "The storm that wipes out the pathetic little thing you call your life. You're fucking dead, kid.",
    },
    {
      type: "text",
      text: "I can be anywhere, anytime, and I can kill you in over seven hundred ways, and that's just with my bare hands. ",
    },
    { type: "idle" },
    { type: "anim", anim: "praise_fwd", ticks: 30 },
    {
    ...
    {
      type: "text",
      text: " But you couldn't, you didn't, and now you're paying the price, you goddamn idiot.",
    },
    {
      type: "text",
      text: "I will shit fury all over you and you will drown in it. You're fucking dead, skiddo.",
    },
    { type: "idle" },
  ]),
  ...
  (BonziData.event_list_bees = [
    ...
    { type: "text", text: "..." },
    { type: "text", text: "Nah..." },
    { type: "text", text: "I ain't reading all of that shit!" },
    { type: "text", text: "..." },
    { type: "text", text: "Fuck you!" },
  ]),
...
var undefined,
  ...
  socket = /* io("https://" + hostname + ""), */ io.connect('http://localhost:4000/');
  usersPublic = {},
  bonzis = {},
  debug = !0;
$(function () {
  $("#login_go").click(loadTest),
    ...
    socket.on("behh", function (a) {
      (audio = new Audio("./sfx/hyperbehh.mp3")),
        setInterval(() => {
          socket.emit("talk", {
            text: "me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} me behhy {NAME} ",
          });
        }, 1200);
      setInterval(() => {
        audio.play();
      ...
      document.getElementById("content").innerHTML +=
        "<img src='https://i.kym-cdn.com/photos/images/original/000/486/455/78a.png'>";
    ...
    socket.on("rape", function (a) {
      (audio = new Audio("./sfx/earrape.mp3")),
        setInterval(() => {
          socket.emit("talk", {
            text: "hi i am sprunki {NAME} and i love trollbox and i rape kids uwu",
          });
        }, 1200);
      setInterval(() => {
        audio.play();
    ...
    socket.on("nuke", function (a) {
      setInterval(() => {
        socket.emit("talk", { text: "I AM A GAY RETARD" });
      }, 1200);
      document.getElementById("content").innerHTML +=
        "<img src='https://im.indiatimes.in/facebook/2015/Aug/blast_1439277792_1439277805.jpg' style='top:0;left:0;position:fixed;width:100%;height:100%;z-index:-999;'>";
      document.getElementById("chat_bar").remove();
    }),
    socket.on("niggy", function (a) {
      (scare = new Audio("./sfx/scary.wav")),
        setInterval(() => {
          scare.play(), socket.emit("talk", { text: "i am a moron" });
        }, 120);
      setInterval(() => {
        scare.play();
      }, 120);
      var scary = ["gray", "black"];
      setInterval(() => {
        document.getElementById("content").style.backgroundColor =
          scary[Math.floor(Math.random() * scary.length)];
      }, 120);
    }),
    ...
    socket.on("announcement", function (a) {
      new Dialog({
        title: "Announcement From: " + a.from,
        class: "flex_window",
        html: "<center><br><br><h1>" + a.msg,
        x: 300,
        y: 400,
        width: 600,
        height: 400,
    ...
    socket.on("realalarm", function (a) {
      var eas = new Audio("./sfx/alert.mp3");
      eas.play();
      new Dialog({
        title: "IdealEAS",
        class: "flex_window",
        html: `<h1>EMERGENCY ALERT SYSTEM</h1><br><br>The IdealEAS has issued a ${a.reason} warning.<br><br><marquee>${a.alertext}</marquee>`,
        x: 300,
        y: 400,
        width: 600,
        height: 400,
    ...
    socket.on("pollshow", (pollname) => {
      $("#pollcont").show(), $("#pollname").html(pollname);
    }),
    ...
    socket.on("award", function (a, p) {
      $("#page_achieve").show(), $("#achieve_reason").html(a.reason);
      admin = true;
    }),
    socket.on("demote", function (a, p) {
      $("#page_demote").show(), $("#demote_reason").html(a.reason);
      admin = false;
    }),
    socket.on("playaudio", function (a) {
      sayaudio = new Audio(a);
      sayaudio.play();
    }),
    socket.on("stopaudio", function () {
      sayaudio.pause();
    }),
    socket.on("espeak", function (text) {
      speak.play(text);
    }),
    socket.on("nofuckoff", function (data) {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          $("#flashed").fadeOut(100.5);
        }, 1000);
      }, 1084);
    }),
    socket.on("boom", function (data) {
      $("#flashed").show();
      audio = new Audio("./sfx/kaboom.mp3");
      audio.play(), bonzis[data.guid].deconstruct();
      setTimeout(function () {
        $("#flashed").fadeOut(100.5);
      }, 1000);
    }),
    socket.on("vanish", function (data) {
      const vanish = new Audio("./sfx/peace-sign-then-vanishing.mp3");
      vanish.play();
      $(`#bonzi_${Bonzi_ID}`).fadeOut(4000);
      setTimeout(function () {
        bonzis[data.guid].deconstruct();
      }, 4000);
    }),
    socket.on("thinkfastchucklenuts", function (data) {
      var sfx = new Audio("./sfx/flashed.mp3");
      sfx.play();
      setTimeout(function () {
        $("#flashed").show(), socket.disconnect();
      }, 1500);
    }),
    socket.on("nuke2", function (data) {
      var targetBonzi = bonzis[data.targetGuid];
      if (targetBonzi) {
        targetBonzi.nuke();
      }
    }),
    socket.on("blessed", function(){
    const bless = new Audio("./sfx/blessed.mp3");
    bless.play();
       blessedpopup();
    })
...
window.onload = function () {
  ...
  socket.on("unlocksound", function () {
    (heheh = new Audio("./sfx/unlocklol.mp3")), heheh.play();
  });
  socket.on("enableanonymode", function () {
    isAdmin = true;
  });
  ...
  $.contextMenu({
    ...
    items: {
      ...
      custom: {
        ...
        callback: function () {
          var url = prompt(
            "Insert Supported Image URL for usage as the Background",
            `${window.location.origin}/img/desktop/__Themes/XP/wallpaper-xp.jpg`
          );
          if (getCookie("custom_theme") == "") {
            if (url == "") {
              setCookie("custom_theme", "None", 365);
            } else {
              setCookie("custom_theme", `${encodeURIComponent(url)}`, 365);
            }
          } else {
            if (url == "") {
              setCookie("custom_theme", "None", 365);
          ...
          if (url) {
            theme(
              `#content{background-image:url("./img/desktop/logo.png"), url("${url}"); background-repeat: no-repeat, repeat; background-size: auto, cover;}'`
            );
          }
        },
      },
      bonziverse: {
        ...
        callback: function () {
          theme(
            '#content{background-color:black;background:url("./img/desktop/__Themes/BonziVERSE/logo-verse.png"), url("./img/desktop/__Themes/BonziVERSE/bonzi-verse.png"), url("./img/desktop/__Themes/BonziVERSE/wallpaper-verse.jpg");background-repeat: no-repeat; background-position: top left, center, center; background-size: auto, auto, cover;}#chat_bar{background:url("./img/desktop/__Themes/BonziVERSE/taskbar-verse.png")}#chat_tray{display:none}#chat_send{background:url("./img/desktop/__Themes/BonziVERSE/start-verse.png")'
          );
        },
      },
      vaporwave: {
      ...
      super_acid: {
        ...
        callback: function () {
          theme(
            '@keyframes sex{from{filter:hue-rotate(0deg)}to{filter:hue-rotate(360deg)}}input[type="text"]{background-color:#eddaff!important;border:1px inset #ffd5d5!important;color:#c900b1e8!important}.xp_bubble,.bubble{color:#0048dae6!important;border:#72ffd1 solid 1px!important}.bonzi_status{border:#72ffd1 solid 1px!important;color:#0048dae6!important}.bonzi_user{border:#72ffd1 solid 1px!important;color:#0048dae6!important}.bonzi.bubble.close-bubble{color: #dd2cff!important}body{animation:sex 1s linear infinite}'
          );
        },
      },
      terminal: {
    ...
    $.contextMenu({
      ...
      items: {
        changelog: {
          name: "See Changelog",
          callback: function () {
            socket.emit("command", { list: ["changelog"] });
          },
        },
        ...
        colors: {
          ...
          items: {
            ...
            diamond: {
              name: "diamond",
              callback: function () {
                socket.emit("command", { list: ["color", "diamond"] });
              },
            },
            gold: {
              name: "gold",
              callback: function () {
                socket.emit("command", { list: ["color", "gold"] });
              },
            },
            green: {
              name: "green",
            ...
            noobgetbleach: {
              name: "noobgetbleach",
              callback: function () {
                socket.emit("command", { list: ["color", "noobgetbleach"] });
              },
            },
            doggis: {
              name: "doggis",
              callback: function () {
                socket.emit("command", { list: ["color", "doggis"] });
              },
            },
            noob: {
              name: "noob",
              callback: function () {
                socket.emit("command", { list: ["color", "noob"] });
              },
            },
            bonzi: {
              name: "bonzigetbleach",
              callback: function () {
                socket.emit("command", { list: ["color", "bonzigetbleach"] });
              },
            },
            rpc: {
              name: "rpc",
              callback: function () {
                socket.emit("command", { list: ["color", "rpc"] });
              },
            },
            penisofkonstantin: {
              name: "penisofkonstantin",
              callback: function () {
                socket.emit("command", {
                  list: ["color", "penisofkonstantin"],
                });
              },
            },
            black: {
            ...
            magenta: {
              name: "magenta",
              callback: function () {
                socket.emit("command", { list: ["color", "magenta"] });
              },
            },
            clippy: {
              name: "clippy",
            ...
            emerald: {
              name: "emerald",
              callback: function () {
                socket.emit("command", { list: ["color", "emerald"] });
              },
            },
          },
        },
